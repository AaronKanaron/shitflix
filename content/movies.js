...
async function loadMovie(movie_id) {
	Promise.all([
			...
			fetch("https://api.themoviedb.org/3/movie/" + movie_id + "?api_key=" + env.api_key + "&language=" + env.lang), 
			fetch("https://api.themoviedb.org/3/movie/" + movie_id + "/watch/providers?api_key=" + env.api_key)
		]).then((responses) => {
			...
			*/
			...
			data[1].hasOwnProperty("results") ? data[1].results.hasOwnProperty("US") ? data[1].results.US.hasOwnProperty("flatrate") ? data[1].results.US.flatrate.map((obj) => { flatrate += "<img src='https://image.tmdb.org/t/p/original" + obj.logo_path + "'>" }) : flatrate = "<p>You cannot stream this movie</p>" : flatrate = "<p>This movie has not come out</p>" : window.location.href = "404.html"
...
function changeActiveButton(){
	document.getElementById("category").childNodes.forEach((obj) => {
